...
function ControlButtons({ className, currentNumber = 1, changeNumber }) {
  ...
  return (
    <div className={cn('flex felx-row justify-between items-center w-[340px] h-[45px]', className)}>
      ...
      <div onClick={() => changeNumber(-1)} className='shadow-custom flex justify-center items-center w-[104px] rounded-[10px] h-full bg-white font-semibold text-primary'>Назад</div>
      <div className='flex flex-row justify-center items-center w-[122px] h-full rounded-[10px] bg-white shadow-custom'>
        <span className='font-sans text-primary'>{currentNumber}</span>
        <span className='font-sans text-black'>/</span>
        <span className='font-sans text-gray'>9</span>
      </div>
      <div onClick={currentNumber != 8  ? () => changeNumber(1) : () => navigate('/found')} className='shadow-custom flex justify-center items-center w-[104px] rounded-[10px] h-full font-semibold text-primary bg-white'>Далее</div>
    </div>
  )
}
